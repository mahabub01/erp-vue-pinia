...
export const useRoleStore = defineStore("role", {
  ...
  actions: {
    getRoleCreateData() {
      ...
      sendGetget("setting/roles/create")
        .then((response) => {
          this.roleCreateData = response.data;
        })
        .catch((error) => {
          appStore.$patch({
    ...
    async getData() {
      const appStore = useAppStore();
      this.isLoading = true;
      sendGetget("setting/roles?page=1&sortField=created_at&sortDir=desc")
        .then((response) => {
    ...
    getSingleRecord(selectedId) {
      const appStore = useAppStore();
      this.blockedProgressLoading = true;
      sendGetget("setting/roles/" + selectedId)
        .then((response) => {
          this.blockedProgressLoading = false;
    ...
    addRecord(payload) {
      const appStore = useAppStore();
      this.blockedProgressLoading = true;
      sendPost("setting/roles", payload)
        .then((response) => {
    ...
    updateRecord(payload, selectedId) {
      const appStore = useAppStore();
      this.blockedProgressLoading = true;
      sendPut("setting/roles/" + selectedId, payload)
        .then((response) => {
          if (response.status !== 200) {
            response.data.errors.forEach((item) => {
              appStore.$patch({
    ...
    removeSingleRecord(t, confirm, selectedId) {
      ...
      confirm.require({
        ...
        accept: () => {
          this.blockedProgressLoading = true;
          sendDelete("setting/roles/" + selectedId)
            .then((response) => {
